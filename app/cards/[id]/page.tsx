...
export default async function CardDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <Link href="/enoncards/cards" className="inline-flex items-center text-[#F59E0B] hover:text-yellow-300 mb-8 transition-colors font-medium">
        ← Zurück zum Katalog
      </Link>

      <div className="grid md:grid-cols-2 gap-12 bg-[#1E293B] rounded-2xl p-8 border border-[#F59E0B]/20">
        <div className="flex justify-center">
          <div className="relative">
            <img
              src="/enoncards/images/placeholder.svg"
              alt={card.name}
              className="w-72 rounded-xl border-2 border-[#F59E0B]/40 shadow-2xl shadow-[#F59E0B]/10"
            />
        ...
        <div className="flex flex-col justify-center">
          ...
          <p className="text-[#F59E0B] text-xl font-semibold mb-1">{card.team}</p>
          ...
          <div className="flex items-center justify-between bg-[#0F172A] rounded-xl p-4 border border-[#F59E0B]/30">
            <span className="text-gray-400 font-medium">Preis</span>
            <span className="text-[#F59E0B] text-3xl font-extrabold">${card.price.toFixed(2)}</span>
          </div>
        </div>
      </div>
    </div>
